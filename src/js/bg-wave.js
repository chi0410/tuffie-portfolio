...
const AMP = 13; // 左右擺幅（px）——擺幅是波浪感的主要來源。8 → 13 讓波形更看得出來
...
const LINE_MID = 'rgba(250, 240, 234, 0.38)'; // 0.28 → 0.38：線本身亮一點，擺動才讀得到
...
export function initBgWave() {
  ...
  function draw(time, animate = true) {
    ...
    for (let x = -GAP; x <= w + GAP; x += GAP) {
      ...
      ctx.globalAlpha = 0.62 + 0.38 * s; // 隨擺動明暗，對比拉大讓脈動更明顯
